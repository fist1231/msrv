...
const searchReviewProposals = (filter) => {
...
const getReviewProposalById = (id) => {
  console.log('$$$$$$$$ getReviewProposalDetailsById id: ' + id);
        ...
        connection.execute(
          ...
          function(err, result)
          {
            if (err) {
              ...
              var reviewProposalDetails = JSON.stringify(result.rows);
              console.log('$$$$$$$$$$ reviewProposalDetails=' + reviewProposalDetails);
              var reviewProposalDetails2 = JSON.parse(reviewProposalDetails);
              //console.log(reviewProposals2);
              resolve(reviewProposalDetails2);
            }
            //console.log(result.metaData); // [ { name: 'DEPARTMENT_ID' }, { name: 'DEPARTMENT_NAME' } ]
            //console.log(result.rows);     // [ [ 180, 'Construction' ] ]
            console.log('*******************************');
...
const resolvers  = {
  Query: {
    ...
    reviewProposalsSearch: (_, { filter }) => searchReviewProposals(filter),
    reviewProposalById: (_, { id }) => getReviewProposalById(id),
  },
  // ReviewProposal: {
  //   id(id) {
